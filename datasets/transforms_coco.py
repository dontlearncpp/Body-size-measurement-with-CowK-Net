...
def crop(image, target, region):
    ...
    if "keypoints" in target:
        ...
        target["keypoints"] = cropped_keypoints.view(target["keypoints"].shape[0], 6, 3)#yang
...
class Normalize(object):
    ...
    def __call__(self, image, target=None):
        ...
        if "keypoints" in target:
            ...
            V[V == 2] = 1
            Z=keypoints[:, :, :2]
            Z = Z.contiguous().view(-1, 2 * 6)####yang
            Z = Z / torch.tensor([w, h] * 6, dtype=torch.float32)####yang

...
#For ResNet-50:
os.environ['EDPOSE_COCO_PATH'] ="/media/test/run/ED-Pose-master-6points/coco"
# --output_dir "logs/coco_r50" -c config/edpose.cfg.py --options batch_size=2 epochs=60 lr_drop=55 num_body_points=6 backbone=resnet50 --dataset_file="coco"
...
def main(args):
    ...
    # im = torch.zeros((2, 3, 608, 1080), device='cuda')
    # maskk = torch.zeros((2, 608, 1080), device='cuda')
    #
    # boxes = torch.rand((1,4), device='cuda')
    # labels = torch.zeros((1), device='cuda')
    # image_id = torch.zeros((1), device='cuda')
    # keypoints = torch.zeros((1,18), device='cuda')
    # area = torch.zeros((1), device='cuda')
    # orig_size = torch.zeros((2), device='cuda')
    # iscrowd = torch.zeros((1), device='cuda')
    # size = torch.tensor([608,1080], device='cuda')
    # targets={
    #     0:{
    #         'boxes':boxes,
    #         'labels':labels,
    ...
    #
    #
    # inputt=NestedTensor(im, maskk)
    # # # inputt.tensors=im
    # # # flops = stat(model,inputt)
    # flops, params = profile(model, (inputt,targets))

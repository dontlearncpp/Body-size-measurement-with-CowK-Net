...
class COCOVisualizer():
    ...
    def addtgt(self, tgt):
        ...
        for box in tgt['boxes'][0,:].unsqueeze(0).cpu():
        ...
        if 'keypoints' in tgt:
            ...
            for idx, ann in enumerate(tgt['keypoints'][0,:].unsqueeze(0)):
            # for idx, ann in enumerate(tgt['keypoints'][:,0:12]):
                if "kpt_bbox" in tgt:
                ...
                with open('test.txt','a') as file:
                    file.write(str(tgt['image_id'].item())+'.jpg'+'\n')
                    x1 = x.astype(int)
                    y1 = y.astype(int)

                    row_strx = ','.join(map(str, x1))
                    row_stry = ','.join(map(str, y1))
                    file.write(row_strx+'\n')
                    file.write(row_stry+'\n')
                for i in range(6):
                    c_kpt = color_kpt[i]
                    plt.plot(x[i], y[i], 'o', markersize=30, markerfacecolor=c_kpt, markeredgecolor='k', markeredgewidth=0.5)
        ax.set_axis_off()
